<%@page import="com.gx.soft.mobile.persistence.domain.GxOaMobileFunction"%>
...
<%
	...
	List<GxOaMobileFunction> functionList = (List<GxOaMobileFunction>)request.getAttribute("functionList"); 
...
<base href="<%=basePath %>">
 <script type="text/javascript">
		...
		function returnJSON() {
			...
			{id:'<%=functionList.get(i).getFunctionId()%>', pId:"1" , name:'<%=functionList.get(i).getFunctionName()%>', checked:<%=functionIdMap.get(functionList.get(i).getFunctionId()) != null ? true : false%> }
			<%if(i < functionList.size()-1){%>,
			<%}%>
			<%}%>,{id:'1',pId:'0',name:'手机端权限菜单'}];
		}
	</script>

<form method="post" action="<%=basePath%>role/mobile-role-permissionUpdate.do" id="role-functionForm" data-toggle="ajaxform"
