        	...
        	<c:forEach items="${page.result}" var="user" varStatus="status">
        		 <tr>
                	 ...
                	 <td>${user.userSex eq '1' ? '男':'女' }</td>
                	  <td><input type="checkbox" name="ids" data-toggle="icheck" value="{ ${userId eq null?'userId':userId}:'${user.userId}', ${userName eq null?'userName':userName}:'${user.userName}'}"></td>
                	 <td>
                    	 <a href="javascript:;" data-toggle="lookupback" data-args="{ ${userId eq null?'userId':userId}:'${user.userId}', ${userName eq null?'userName':userName}:'${user.userName}'}" class="btn btn-blue" title="选择本项" data-icon="check">选择</a>
                	 </td>
           		 </tr>
