...
+function ($) {
    ...
    Bjuiajax.prototype.ajaxForm = function(options) {
        ...
        var $form     = this.$element,
            ...
            onsubmit  = options && options.onsubmit,//20150915 wyy -update
            enctype   = $form.attr('enctype')
        
        options = $.extend({}, that.options, typeof options == 'object' && options)
        if (callback) callback = callback.toFunc()
        var onsubmitFlag = true;//20150915 wyy -update
        if (onsubmit){ 
        	onsubmit = onsubmit.toFunc();
    ...
    Bjuiajax.prototype.doAjax = function(options) {
        ...
        var todo     = function() {
            $element.doAjax({type:options.type, loadingmask:options.loadingmask,url:options.url, data:options.data,callback:callback ? callback : $.proxy(function(data) {that.ajaxCallback(data)}, that)})
