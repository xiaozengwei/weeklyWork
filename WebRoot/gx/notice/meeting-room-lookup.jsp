...
<div class="bjui-pageContent" id="outdiv">
    ...
    <table class="table table-bordered" data-selected-multi="true" id="metTab">
        ...
        <c:forEach items="${meetList}" var="meet">
            <tr>
                <td rowspan="2" align="center"><input type="radio" name="ids" data-toggle="icheck" value="${meet.meetingRoomName}"></td>
    ...
    <div style="padding:5px 0px; width:100%; text-align:center;"><button type="button" class="btn-blue" id="btn-user-select">确定</button></div>
</div>
<script>
    ...
    $(function(){
        ...
        $("#btn-user-select",$.CurrentDialog).on("click",function(){
            ...
            var users_selected_td = $(":radio:checked");
            var v_org_names ="";
            var v_org_ids = "";
            if(users_selected_td.length < 1){
                $(this).alertmsg('info', '请选择地点');
                return;
            }else{
                var addr = users_selected_td.val();
                var orgNames = addr.split(",");
                for(var i = 0;i < orgNames.length; i++) {

                    v_org_names+= orgNames[i] +",";
                ...
                $.CurrentNavtab.find(':input').each(function() {
                    ...
                    if (name == "meetingArrangement[${group}].meetingRoomName") {
