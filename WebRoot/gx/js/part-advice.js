$(function(){
	...
	$(".form-advice-list",$.CurrentNavtab).each(function(){
		...
		var table_name="gx_oa_sw";
		var _url =_BASE_PATH + 'signature/signlist.do';
		var data = {
				rowId : fwId,
				signControlName: contol_name,
				tableName:table_name
		};
		$.get(_url,data,function(json){
			var d_html ="";
			$.each(json,function(i,sign){
				...
				createTime = d.format("yyyy-MM-dd hh:mm:ss") ;
				 ...
				 d_html +="<div ><div class=\"form-advice\"><label class=\"control-label x100\">"+optionContext+"</label></div><div class=\"form-advice_user_and_date\">";
				d_html += getSignatureNote(creatorId,creatorName,true);
				d_html += "<label class=\"control-label x100\" style=\"font-size:12px;\">"+createTime+"</label></div></div>";
