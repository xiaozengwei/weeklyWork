...
function formatDateWithChunk(date, chunk) {
	...
	if (typeof chunk === 'string') { // a literal string
		...
		if (tokenOverrides[token]) {
			retunRes= tokenOverrides[token](date); // use our custom token
		}
		retunRes=  oldMomentFormat(date, token);
	}
		...
		//if (maybeStr.match(/[1-9]/)) {
			//retunRes= maybeStr;
		//}
		if(maybeStr.indexOf("00")){//处理8点00，截取为8点
			maybeStr = maybeStr.substring(0,maybeStr.length-2);
		}
		retunRes = maybeStr;
	}
...
var momComputableOptions = {
	...
	// Produces format strings like "h(:mm)a" -> "6pm" / "6:30pm"  Ah点mm
