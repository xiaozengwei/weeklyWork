    ...
    public static Criterion[] buildCriterion(List<PropertyFilter> filters) {
        ...
        for (PropertyFilter filter : filters) {
            ...
            if (!filter.hasMultiProperties()) {
            	logger.info(filter.getPropertyName()+"-"+filter.getMatchValue()+"-"+ filter.getMatchType());
               ...
               /* for (String param : filter.getPropertyNames()) {
                ...
                //2015-08-25wyy-update 修复在or情况下，value值查询的问题 
                String valuesObj = (String) filter.getMatchValue();
                String [] values = valuesObj.split(",");
                String [] propertyNames = filter.getPropertyNames();
                for(int i = 0 ;i < propertyNames.length;i++){
                	String param = propertyNames[i];
                	String value = values[i];
                	Criterion criterion = buildCriterion(param,
                			value, filter.getMatchType());
                    disjunction.add(criterion);
                }
